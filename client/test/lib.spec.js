...
import { ABIStorage, Blob, File } from 'abi.storage'
...
describe('client', () => {
  ...
  it('interface', () => {
    assert.equal(typeof ABIStorage, 'function')
    const client = new ABIStorage({ token: 'secret' })
    assert.ok(client instanceof ABIStorage)
    ...
    assert.equal(typeof ABIStorage.storeBlob, 'function')
    assert.equal(typeof ABIStorage.storeDirectory, 'function')
    assert.equal(typeof ABIStorage.status, 'function')
    assert.equal(typeof ABIStorage.delete, 'function')
  })
  describe('upload', () => {
    it('upload blob', async () => {
      const client = new ABIStorage({ token, endpoint })
